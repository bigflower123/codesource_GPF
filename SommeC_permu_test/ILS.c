...
int *ILS(int *M, int n, int m,int *ils_itr,int *ils_itr_sol){
	int i,sumP=0;
	...
	*(dupPI0+i)=p->job[0];
	...
	for(i=0;i<n;i++)	
		printf("%-4d",dupPI0[i]);
	printf("LS la somme de date de fin est:  %d\n",calculateCbarre(M, m,n, dupPI0, n));*/
	// iRZ(PI0)
	for(i=0;i<n;i++){
		PI[i]=dupPI0[i];
	}	
	...
	*ils_itr_sol+=nbr_itr_resulat;  // enregistrer la somme du nombre les intération de meilleur solution 
	*ils_itr+=nbr_itr;              // enregistrer la somme de l'iteration total
