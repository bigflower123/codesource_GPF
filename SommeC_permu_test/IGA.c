...
int *IGA(int *M, int n, int m,int *iga_itr, int *iga_itr_sol){
	int i,sumP=0;
	...
	*iga_itr_sol+=nbr_itr_resulat;    // enregistrer la somme du nombre les intération de meilleur solution
	*iga_itr+=nbr_itr;                // enregistrer la somme de l'iteration total
...
int *destruction_construction(int *M,int *PI,int n,int m){
	...
	for(k=0;k<d;k++)   // d jobs
	{
		...
		tmp=calculateCbarre(M, m, n, resDup, n-d+k+1);
		if(tmp<sommeC)
		{
		    sommeC=tmp;
		    pos=j;
